...
function ChatWindow(props) {
  const [messages, setMessages] = useState([
    {
      id: 1,
      text: "Hello! I'm Alvin. How can I help you today?",
      sender: "bot",
      timestamp: new Date(),
    },
  ]);
  ...
  // Offset chat above any visible bottom navs on mobile
  useEffect(() => {
    const calcBottomOffset = () => {
      const bars = Array.from(document.querySelectorAll(".mobile-bottom-nav"));
      const visibleHeights = bars
        .filter((el) => window.getComputedStyle(el).display !== "none")
        .reduce((sum, el) => sum + el.offsetHeight, 0);
      const padding = 12; // breathing room above the navs
      setBottomOffset(visibleHeights + padding);
    };
    ...
    if (supportsResizeObserver) {
      document.querySelectorAll(".mobile-bottom-nav").forEach((el) => {
        const ro = new ResizeObserver(() => calcBottomOffset());
        ro.observe(el);
        observers.push(ro);
      });
    }

    window.addEventListener("resize", calcBottomOffset);
    return () => {
      window.removeEventListener("resize", calcBottomOffset);
  ...
  return (
    <div
      className={`chat-container ${isCollapsed ? "collapsed" : ""} ${
        props?.user?.role
      }-wrapper ${props?.user ? "active" : "inactive"}`}
      style={{ "--bottom-ui-offset": `${bottomOffset}px` }}
    >
      {/* Header */}
      <div
