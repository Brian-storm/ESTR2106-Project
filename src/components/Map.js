...
function Map() {
  ...
  useEffect(() => {
    ...
    if (venueId) {
      ...
      const venueIndex = venues.findIndex((v) => v.venueId === venueId);
      if (venueIndex !== -1) {
  ...
  const fetchCurrentVenueComments = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        "/api/locations/" + venues[selectedVenue]._id + "/comments"
      );
  ...
  return (
    <div className="w-100 h-100">
      ...
      >
        ...
        {selectedVenue !== null && (
          <>
            <div className="mt-0 mb-2 fs-5 fw-bold">
              {venues[selectedVenue].name}
            </div>
            {venues[selectedVenue].address && (
              <p>
                <strong>Address:</strong> {venues[selectedVenue].address}
              </p>
            )}
            {venues[selectedVenue].latitude &&
              venues[selectedVenue].longitude && (
                <p>
                  <strong>Coordinates:</strong>{" "}
                  {venues[selectedVenue].latitude.toFixed(4)},{" "}
                  {venues[selectedVenue].longitude.toFixed(4)}
                </p>
              )}
                ...
                currentVenueComments.map((comment, index) => (
                  ...
                  >
                    <div>
                      ...
                      {new Date(comment.date).toLocaleString()} wrote:
                    </div>
